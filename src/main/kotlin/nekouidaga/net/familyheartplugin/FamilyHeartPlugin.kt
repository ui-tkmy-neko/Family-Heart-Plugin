...
class FamilyHeartPlugin:JavaPlugin(){
    ...
    listOf("messages.yml","gui.yml","buffs.yml","penalties.yml").forEach{if(!dataFolder.resolve(it).exists())saveResource(it,false)};db=DatabaseManager(logger);try{db.connect(config)}catch(e:Exception){logger.severe("[FamilyHeart] SQLite初期化失敗: ${e.message}");server.pluginManager.disablePlugin(this);return};val players=PlayerDao();val rdao=RelationshipDao();val cache=RelationshipCache();val rel=RelationshipService(this,db,rdao,players,cache);val settings=Settings(this);val econ=EconomyService(this);econ.hook();val req= nekouidaga.net.familyheartplugin.request.RequestService(db,RequestDao(),rel,settings,econ,players);val pen=PenaltyService(this,db,PenaltyDao(),rel);val messages=Messages(this);val act=ActionService(this,db,ActionDao(),rel,req,settings,messages);val audit=AuditService(db,AuditDao());val buffs=BuffService(this,db,act,rel,pen,settings);
